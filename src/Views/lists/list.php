	...
	<div class="header-wrapper">
		<header class="header">
			<div class="inner for-todo-form">
				...
				<h2>
					{{!empty($navData['list']) ? $navData['list']->title : 'All lists'}}
					<a href="/{{$navData['teamSlug']}}/{{$navData['listSlug']}}/edit/" class="link">
					[[edit]]
					</a>
				</h2>
			</div>
	...
	<?php else: ?>
		<?php if (!empty($tasksDone) && sizeof($tasksDone) >= 1): ?>
			<section class="container center mt-4">
				<a href="/{{$navData['teamSlug']}}/{{$navData['listSlug']}}/show-done/">
					[[show]] {{' ' . sizeof($tasksDone) . ' '}} [[done_todos]]
				</a>
			</section>
		<?php endif;?>
	<?php endif;?>
